...
-- ============================================================================
-- 23. AUTOMATIC ACCOUNT_ID BACKFILL & INSERT TRIGGERS
-- ============================================================================
...
UPDATE public.transactions t SET account_id = p.account_id FROM public.profiles p WHERE t.user_id = p.id AND t.account_id IS NULL AND p.account_id IS NOT NULL;
UPDATE public.invoices i SET account_id = p.account_id FROM public.profiles p WHERE i.user_id = p.id AND i.account_id IS NULL AND p.account_id IS NOT NULL;
UPDATE public.bills b SET account_id = p.account_id FROM public.profiles p WHERE b.user_id = p.id AND b.account_id IS NULL AND p.account_id IS NOT NULL;
UPDATE public.clients c SET account_id = p.account_id FROM public.profiles p WHERE c.user_id = p.id AND c.account_id IS NULL AND p.account_id IS NOT NULL;
UPDATE public.suppliers s SET account_id = p.account_id FROM public.profiles p WHERE s.user_id = p.id AND s.account_id IS NULL AND p.account_id IS NOT NULL;
UPDATE public.products pr SET account_id = p.account_id FROM public.profiles p WHERE pr.user_id = p.id AND pr.account_id IS NULL AND p.account_id IS NOT NULL;
UPDATE public.quotations q SET account_id = p.account_id FROM public.profiles p WHERE q.user_id = p.id AND q.account_id IS NULL AND p.account_id IS NOT NULL;
UPDATE public.employees e SET account_id = p.account_id FROM public.profiles p WHERE e.user_id = p.id AND e.account_id IS NULL AND p.account_id IS NOT NULL;
UPDATE public.projects proj SET account_id = p.account_id FROM public.profiles p WHERE proj.user_id = p.id AND proj.account_id IS NULL AND p.account_id IS NOT NULL;
UPDATE public.tickets tk SET account_id = p.account_id FROM public.profiles p WHERE tk.user_id = p.id AND tk.account_id IS NULL AND p.account_id IS NOT NULL;
UPDATE public.bugs bg SET account_id = p.account_id FROM public.profiles p WHERE bg.user_id = p.id AND bg.account_id IS NULL AND p.account_id IS NOT NULL;
UPDATE public.documents doc SET account_id = p.account_id FROM public.profiles p WHERE doc.user_id = p.id AND doc.account_id IS NULL AND p.account_id IS NOT NULL;
UPDATE public.shareholders sh SET account_id = p.account_id FROM public.profiles p WHERE sh.user_id = p.id AND sh.account_id IS NULL AND p.account_id IS NOT NULL;
UPDATE public.client_tracking ct SET account_id = p.account_id FROM public.profiles p WHERE ct.user_id = p.id AND ct.account_id IS NULL AND p.account_id IS NOT NULL;
UPDATE public.lead_tracking lt SET account_id = p.account_id FROM public.profiles p WHERE lt.user_id = p.id AND lt.account_id IS NULL AND p.account_id IS NOT NULL;
UPDATE public.vendor_payouts vp SET account_id = p.account_id FROM public.profiles p WHERE vp.user_id = p.id AND vp.account_id IS NULL AND p.account_id IS NOT NULL;
...
-- 24. ENTERPRISE ERP TABLES (Sales Orders, PO, GRN, IT Assets, Contracts, Timesheets, HR, Workflows)
...
BEGIN
  FOREACH t IN ARRAY erp_tables LOOP
    ...
    EXECUTE format('CREATE POLICY %I_tenant_isolation ON public.%I FOR ALL TO authenticated USING (account_id = public.current_account_id() OR public.is_super_admin()) WITH CHECK (account_id = public.current_account_id() OR public.is_super_admin());', t, t);
    EXECUTE format('DROP TRIGGER IF EXISTS trg_auto_set_account_id ON public.%I;', t);
    EXECUTE format('CREATE TRIGGER trg_auto_set_account_id BEFORE INSERT ON public.%I FOR EACH ROW EXECUTE FUNCTION public.auto_set_account_id();', t);
  END LOOP;
END $$;
