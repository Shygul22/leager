...
-- 4.7 Synchronize Auth Users into Public Profiles gracefully
...
BEGIN
  ...
  -- Check if profile already exists with this EMAIL (e.g. legacy/seed/different UUID)
  ...
  RETURN NEW;
EXCEPTION WHEN OTHERS THEN
  -- Never crash auth transaction if profile sync encounters any edge case
  RAISE WARNING 'handle_auth_user_sync error suppressed: %', SQLERRM;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Re-attach triggers to auth.users safely
